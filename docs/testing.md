...
| `AppSettingsDefaultsTest` | `NotificationSettings()` and `CommandSettings()` data-class defaults for reliability-critical booleans; JSON absent-key fallback values match data-class defaults; JSON explicit `false` survives parsing unchanged (backward compat for users who opted out) |
